...
<head>
	...
	<title>Relatorio - RR Imagem Masculina</title>
				...
				<div class="row">
								...
								<div class="card border p-3 w-100">
									...
									<div class="card-body p-0 mt-4">
										...
										<ul class="nav nav-pills-primary-border-start flex-column">
											...
											<li class="nav-item">
												<a class="nav-link active" href="account-order.php"><i class="bi bi-basket fa-fw me-2"></i>Relatorio Geral</a>
											</li>
											<li class="nav-item">
					...
					<div class="col-lg-8 col-xl-9 ps-lg-4 ps-xl-6">
						...
						<div class="d-flex justify-content-between align-items-center mb-5 mb-sm-6">
							<!-- Title -->
							<h1 class="h3 mb-0">Relatorio de agendamentos</h1>
						...
						<?php
						// Defina $filter_option como 'all' por padrão
						$filter_option = 'all';

						// Verifique se $_POST['filterOption'] está definido e atribua seu valor a $filter_option, se estiver
						if (isset($_POST['filterOption'])) {
							$filter_option = $_POST['filterOption'];
						}
						?>
						<div class="card bg-transparent">
							<!-- Card header -->
							<div class="card-header bg-transparent border-bottom d-flex justify-content-between align-items-center p-0 pb-3">
								<h6 class="card-title mb-0">Seus agendamentos</h6>
								<div class="col-md-3 ms-auto">
									<!-- Short by filter -->
									<a id="submitButton" class="btn btn-light mb-2 w-100"><i class="bi bi-file-earmark-pdf me-2"></i>Download Pdf</a>
									<form id="filterForm" method="POST" action="account-order.php">
										<select class="form-select js-choice" id="filterSelect" aria-label=".form-select-sm" name="filterOption">
											<option value="all" <?php echo ($filter_option == 'all') ? 'selected' : ''; ?>>Todos</option>
											<option value="semanal" <?php echo ($filter_option == 'semanal') ? 'selected' : ''; ?>>Semanal</option>
											<option value="mensal" <?php echo ($filter_option == 'mensal') ? 'selected' : ''; ?>>Mensal</option>
											<option value="hoje" <?php echo ($filter_option == 'hoje') ? 'selected' : ''; ?>>Hoje</option>
										</select>
									</form>
							...
							// Calcular o deslocamento para o conjunto de registros
							$deslocamento = ($pagina - 1) * $registros_por_pagina;

							// Obter o ID do barbeiro da sessão
							$barber_id = $_SESSION["barber_id"];

							// Inicializar a variável $appointments como um array vazio
							$appointments = array();

							// Definir a condição do filtro com base na opção selecionada no formulário

							$filter_option = $_POST['filterOption'] ?? 'all';

							// Consulta SQL para recuperar os registros do banco de dados para o barbeiro específico, com base na opção de filtro selecionada
							$sql = "SELECT * FROM appointment_list WHERE barber_id = ?";
							switch ($filter_option) {
								case 'semanal':
									$sql .= " AND DATE(schedule) BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 WEEK) AND CURDATE()";
									break;
								case 'mensal':
									$sql .= " AND MONTH(schedule) = MONTH(CURDATE())";
									break;
								case 'hoje':
									$sql .= " AND DATE(schedule) = CURDATE()";
									break;
								default:
									// Nenhuma condição de filtro adicionada para listar todos os agendamentos
									break;
							}
							$sql .= " AND fullname IS NOT NULL AND fullname != '' ORDER BY CAST(schedule AS DATETIME) DESC LIMIT ?, ?";


							$stmt = $conexao->prepare($sql);
							$stmt->bind_param('iii', $barber_id, $deslocamento, $registros_por_pagina);
							$stmt->execute();
							$result = $stmt->get_result();
						...
						// Array para armazenar os compromissos
$appointments = array();
...
if ($result->num_rows > 0) {
    ...
    while ($appointment = $result->fetch_assoc()) {
        ...
        // Verificar se o serviço é 'invalido'
        if ($service_id === 'invalido') {
            // Se for 'invalido', obter o custo do combo com base no ID do serviço
            $service_id = $appointment['combo'];
            $sql_service = "SELECT price FROM service_list_combos WHERE combo_id = ?";
            $stmt_service = $conexao->prepare($sql_service);
            $stmt_service->bind_param('i', $service_id);
            $stmt_service->execute();
            $result_service = $stmt_service->get_result();

            // Verificar se o combo foi encontrado
            if ($result_service->num_rows > 0) {
                $row_service = $result_service->fetch_assoc();
                $cost = $row_service['price']; // Usar 'price' em vez de 'cost'
                // Armazenar o custo do combo no array $appointment
                $appointment['cost'] = $cost;
            } else {
                // Se o combo não for encontrado, atribuir um custo padrão
                $appointment['cost'] = 0;
            }
        } else {
            // Se o serviço não for 'invalido', obter o custo do serviço com base no ID do serviço
            $sql_service = "SELECT cost FROM service_list WHERE id = ?";
            $stmt_service = $conexao->prepare($sql_service);
            $stmt_service->bind_param('i', $service_id);
            $stmt_service->execute();
            $result_service = $stmt_service->get_result();
            ...
            if ($result_service->num_rows > 0) {
                $row_service = $result_service->fetch_assoc();
                $cost = $row_service['cost'];
                // Armazenar o custo do serviço no array $appointment
                $appointment['cost'] = $cost;
            } else {
                // Se o serviço não for encontrado, atribuir um custo padrão
                $appointment['cost'] = 0;
            }
        }

        // Adicionar o compromisso, incluindo o custo, à matriz $appointments
        $appointments[] = $appointment;
										...
										<div class="row align-items-md-center">
											...
											<div class="col-5 col-md-2">
												<div class="bg-light p-2 rounded-2">
													<img src="https://i.imgur.com/CHBNE96.png" alt="">
												</div>
											</div>
											...
											<div class="col-7 col-md-10">
												<div class="row g-2 align-items-center">
													...
													<div class="col-md-6 mb-3 mb-md-0">
														<p class="heading-color fw-normal small mb-2">N° ID: <span class="text-primary">#<?php echo $appointment['id']; ?></span></p>
														<h6 class="mb-2"><a href="#"><?php echo $appointment['fullname']; ?></a></h6>
														<!-- List -->
														<ul class="nav nav-divider small align-items-center">
															<li class="nav-item">F/pagamento: <b><?php echo strtoupper($appointment['pagamento']); ?></b></li>
															<li class="nav-item">Data de agendamento: <b><?php echo date('d/m/Y', strtotime($appointment['schedule'])); ?></b></li>
													...
													<div class="col-md-4 text-md-end ms-auto">
														<p class="fw-semibold mb-1 mb-md-3">
															<?php if ($appointment['status'] == 0) : ?>
																<i class="bi bi-x-circle-fill text-danger me-1"></i><span style="color: #ff6961;">Pendente</span>
															<?php elseif ($appointment['status'] == 1) : ?>
																<i style="color: #3065AC;" class="bi bi-check-circle-fill  me-1"></i><span style="color: #3065AC;">Confirmado</span>
															<?php elseif ($appointment['status'] == 2) : ?>
																<i class="bi bi-patch-check-fill text-warning me-2"></i><span style="color: #fdfd96;">Concluído</span>
															<?php endif; ?>
														</p>
														...
														<h5 class="mt-1 mb-0">R$ <?php echo $appointment['cost']; ?> REAIS</h5>
													</div>
												</div>
											</div>
										</div>
							...
							$stmt->close();
							$conexao->close();
							$total_paginas = ceil(count($appointments) / $registros_por_pagina);
							ceil(count($appointments));
							...
							<div class="card-footer bg-transparent text-end p-0">
								<!-- Pagination -->
								<ul class="pagination pagination-primary-soft list-unstyled d-flex justify-content-sm-end flex-wrap mb-0">
									<!-- Previous button -->
									<li class="page-item <?php echo ($pagina <= 1) ? 'disabled' : ''; ?>">
										<a class="page-link" href="?page=<?php echo max($pagina - 1, 1); ?>">Anterior</a>
									</li>
									...
									<?php endfor; ?>
									<!-- Next button -->
									<li class="page-item <?php $total_paginas = 50;
															echo ($pagina >= $total_paginas) ? 'disabled' : ''; ?>">

										<a class="page-link" href="?page=<?php echo ($pagina < $total_paginas) ? $pagina + 1 : $total_paginas; ?>">Proxima</a>
									</li>
								</ul>
							</div>
				...
				<div class="col-lg-8 col-xxl-7">
					<div class="row g-4">
						...
						<div class="col-md-4">
							...
							<div class="row g-2 align-items-center mt-2 mb-4 mb-sm-5">
...
Footer END -->
	...
	<!-- Coloque isso no final do seu arquivo HTML, antes do fechamento da tag body -->

	<script>
		document.addEventListener('DOMContentLoaded', function() {
			// Detectar mudança na seleção do filtro e enviar o formulário
			document.getElementById('filterSelect').addEventListener('change', function() {
				console.log('Seleção de filtro alterada'); // Verifica se o evento de mudança está sendo acionado
				document.getElementById('filterForm').submit();
			});

			// Adicionar um evento para o envio do formulário
			document.getElementById('filterForm').addEventListener('submit', function() {
				console.log('Formulário enviado'); // Verifica se o evento de envio do formulário está sendo acionado
			});
		});
	</script>
